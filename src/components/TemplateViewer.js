  ...
  methods: {
    async renderElement(h, node, parentChildren) {
      ...
      for (const child of node.children)
        await this.renderNode(h, child, children, node[Scope]);
    ...
    async renderNode(
      ...
      if (node.dataProps)
        node.dataProps = node.dataProps.map(item => ({ ...item })); // copy

      if (overrideProps)
        node.props = node.props.concat(
          overrideProps.map(item => ({ ...item }))
        );
      if (overrideClass)
        node.class = node.class.concat(
          overrideClass.map(item => ({ ...item }))
        );
      if (overrideStyle)
        node.style = node.style.concat(
          overrideStyle.map(item => ({ ...item }))
        );
      ...
      for (let index = 0; index < repeat.length; index++) {
        ...
        switch (node.type) {
          case "element":
            await this.renderElement(h, node, parentChildren);
            break;
          case "text":
            await this.renderText(h, node, parentChildren);
            break;
          case "image":
            await this.renderImage(h, node, parentChildren);
            break;
          case "template":
            await this.renderTemplate(h, node, parentChildren);
            break;
          case "switch":
            await this.renderSwitch(h, node, parentChildren);
            break;
          case "context":
            await this.renderContext(h, node, parentChildren);
            break;
        }
      }
    }
