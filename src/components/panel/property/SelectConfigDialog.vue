  ...
  <q-dialog :value="value" @input="$emit('input', $event)">
    <q-card v-contextmenu="getOptionCM(null)">
      <q-card-section class="row items-center">
        ...
        <q-space />
        <q-btn icon="add" flat round size="sm" color="primary" @click="onCreateOption" />
      </q-card-section>
      <q-card-section v-if="selectProp">
        <draggable v-model="selectProp.options" :animation="100" handle=".handle">
          <PropExpr
    ...
    getOptionCM (option) {
      return {
        ...
        menu: [
          ...
          { separator: true },
          { label: "Rename", handler: this.onUpdateOptionName, disable: option == null},
          { label: "Delete", handler: this.onDeleteOption, class:"text-negative", disable: option == null}
        ]
      }
    },
