...
export default Vue.extend({
  ...
  computed: {
    nodes () {
      return this.value?.[this.rootKey]?.[this.childrenKey] ?? []
    },
    ...
    meta () {
      ...
      const travel = (node, parent) => {
        ...
        if (isParent === true) {
          ...
          m.children = node[this.childrenKey].map(n => travel(this.value[n], m))
      ...
      // this.nodes.forEach(nodeKey => travel(this.value[nodeKey], null))
      travel(this.value[this.rootKey], null)
      return meta
    }
  },
  ...
  methods: {
        ...
        travel = node => {
          ...
          if (node[this.childrenKey]) {
            if (node.expandable !== false && node.disabled !== true) {
              ...
              node[this.childrenKey].forEach(key => travel(this.value[key]))
            }
          }
        }
    ...
    __onChanged (key, newChildren) {
      if (!this.__valueChanged) { // if not touched before
        this.__valueChanged = true

        // copy 2 level this.value
        this.__updatedValue = {}
        for (let key in this.value) {
          this.__updatedValue[key] = {...this.value[key]}
        }

        this.$nextTick(() => {
          this.__valueChanged = false
          this.$emit('input', this.__updatedValue)
        })
      }

      this.__updatedValue[key][this.childrenKey] = newChildren
    },
  ...
  render (h) {
    ...
    return h(
      'draggable', {
        class: this.classes,
        props: {
          value: this.value?.[this.rootKey]?.[this.childrenKey] ?? [],
          // options: this.options,
        },
        attrs: {...this.options},
        on: {
          // change: value => {
          //   this.__onChanged(this.rootKey, value)
          // },
          input: value => {
            this.__onChanged(this.rootKey, value)
          },
